...
router.get("/:user_id/posts", (req, res) => {
    ...
    User.findById(req.params.user_id, (err, foundUser) => {
        ...
        Post.find().populate('category').where('author.id').equals(foundUser._id).exec((err, posts) => {
            if (err) {
                req.flash("error", "User can't be accessed");
                res.redirect('/')
                console.log(err);
...
//CREATE PAGE - NEW POST AFFILIATE
router.get('/:user_id/posts/:id/affiliates/new', async(req, res) => {
    ...
    User.findById(req.params.user_id, (err, user) => {
        Post.findById(req.params.id, (err, post) => {
            ...
            res.render('admin/affiliate_create', { post, user });
        });
    });
...
//AFFILIATES PRODUCTS
router.post("/:user_id/posts/:id/affiliates", async(req, res) => {

    const user = await User.findById(req.params.user_id);
    if (!user) {
        res.flash('error', 'User of post not found')
    }
    const post = await Post.findById(req.params.id);
    if (!post) {
        res.flash('error', 'Post not found')
    } else {
        const affiliate = new Affiliate(req.body.affiliate);
        post.affiliates.push(affiliate);
        await affiliate.save();
        await post.save();
        req.flash('success', 'Created new affiliate Product!');
        res.redirect(`/posts/${post._id}`);
    }
});

//UPDATE AFFILIATE
router.put("/:user_id/posts/:id/affiliates/:AffiliateId/edit", async(req, res) => {
    User.findById(req.params.user_id, async(err, foundUser) => {

        const affiliateProduct = await AffiliateProduct.findById(req.params.AffiliateId);

        try {
            const updatedAffiliateProduct = await AffiliateProduct.findByIdAndUpdate(
                req.params.AffiliateId, {
                    $set: req.body,
                }, { new: true }
            );
            res.status(200).json(updatedAffiliateProduct);
        } catch (err) {
...
//DELETE AFFILIATE
router.delete("/:user_id/posts/:id/affiliates/:AffiliateId", async(req, res) => {
    User.findById(req.params.user_id, async(err, foundUser) => {
...
//GET AFFILIATE
router.get("/:user_id/posts/:id/affiliates/:affiliateId", async(req, res) => {
    User.findById(req.params.user_id, async(err, foundUser) => {

        try {
            const singleAffiliateProduct = await AffiliateProduct.findById(req.params.AffiliateId);
            res.render("admin/affilaite_show", { singleAffiliateProduct: singleAffiliateProduct })
            res.status(200).json(singleAffiliateProduct);
        } catch (err) {
            res.status(500).json(err);
        }
    });
...
//GET ALL AFFILIATES
router.get("/user_id/posts/:id/affiliates", async(req, res) => {
    User.findById(req.params.user_id, async(err, foundUser) => {

        try {
            affiliates = await AffiliateProduct.find();
            res.render('admin/affiliate_index', {
